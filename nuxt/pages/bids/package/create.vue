        ...
        <a-col :xs="22" :md="8" :lg="8">
          <a-card title="Настройка получения клиентов">
            <div class="bid-form">
              ...
              <div class="bid-item">
                <div class="bid-label">
                  Cколько платящих клиентов вы хотите получить
                </div>
                <a-input-number
                  ...
                  <div class="bid-recommend">
                    <span class="bid-recommend-c">
                      <a
                        ...
                        @click="
                          (e) => {
                            costPerRate = computedRecommend.costPerRate;
                            this.handlecostPerRate(e);
                          }
                        "
                      >
                        Рекомендуемая ставка
                        {{ computedRecommend.costPerRate }} ₽
                      </a>
                    </span>
                    <span class="bid-recommend-c">
  ...
  created() {
    ...
    app.$axios
      ...
      .then(({ data }: any) => {
        ...
        app.insuranceRate = data.options.insurance_rate;
      })
      .catch((_err: any) => {
        console.error(_err);
      });
  ...
  computed: {
    ...
    computedFunel() {
      ...
      const _count = Math.ceil(
        app.countEmployee <= 0 ? 100 : (app.countEmployee * 100 / app.direction.conversion_meetings)
      );
      const _mettings = Math.ceil(
        _count * (app.direction.conversion_meetings / 100)
      );
      const _deals = Math.ceil(
        _mettings * (app.direction.conversion_contract / 100)
      );

      const _total = Math.ceil(_deals * app.direction.average_check);
      const _totalBudget = Math.ceil(_count * app.costPerRate);

      const conversionContract = app.isDealGrant
        ? app.direction.conversion_contract / 100 * _totalBudget
        : 0;

      const insuranceRate = app.isInsurance ? app.insuranceRate / 100 * _totalBudget : 0;
      const totalContract = Math.ceil(_count * (app.direction.conversion_contract / 100));
      const costContract = Math.ceil(_totalBudget / totalContract);
      ...
      return {
        ...
        totalBudget: isNaN(_totalBudget) ? 0 : _totalBudget + conversionContract + insuranceRate,
        totalContract: isNaN(totalContract) ? 0 : totalContract,
        costContract: isNaN(costContract) ? 0 : costContract,
      };
    },
    computedIsRegions() {
      const app: any = this;
