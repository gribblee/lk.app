...
class Tinkoff {
     ...
     */
    public function __construct($acquiring_url, $terminal_id, $secret_key, $init_credit_url = '') {
	$this->acquiring_url  = $acquiring_url;
        $this->terminal_id    = $terminal_id;
        $this->secret_key     = $secret_key;
        $this->init_credit_url = $init_credit_url;
        $this->setupUrls();
    }
    ...
    public function creditCreate(array $payment, array $items)
    {
        if ( !$this->creditArrayChecked($payment) ) {
            $this->error = 'Incomplete payment data';
            return FALSE;
        }
        ...
        foreach ($items as $item) {
            if ( !$this->itemsCreditArrayChecked($item) ) {
                $this->error = 'Incomplete items data';
                return FALSE;
            }
            $payment['Items'][] = [
        ...
        if ($this->sendCreditRequest($this->init_credit_url, $params)) {
            return (object)[
                'id' => $this->credit_id,
                'link' => $this->credit_url,
    ...
    private function sendCreditRequest($path, array $args) {
        ...
        if ($curl = curl_init()) {
            ...
            if($json) {
                if ( $this->errorsCreditFound() ) {
                    return FALSE;
			
                } else {                    
                    $this->credit_id       = @$json->id;
                    $this->credit_url      = @$json->link; 
    ...
    private function errorsCreditFound():bool {
        $response = json_decode($this->response, TRUE);
        $error_code = 0;
        if (isset($response['errors'])) {
            $error_msg = $response['errors'][0];
            $error_code = 1;
        } else {
            $error_code = 0;
        }
    ...
    private function creditArrayChecked(array $array_for_check)
    {
        $keys = ['ShopId', 'Sum'];
    ...
    private function itemsCreditArrayChecked(array $array_for_check){
